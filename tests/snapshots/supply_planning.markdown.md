...
- check_coman_availability (information, to co_manufacturing): quantum=ComanAvailabilityQuery, returns=ComanAvailability
    ...
    axioms:
      - viable_coman_shift [flow, advisory]
          Shifting production to a co-manufacturer (shift_to_coman) is a structurally viable resolution path only when a qualified co-man has capacity in the requested window — ComanAvailability.is_qualified AND has_capacity must both be true. An unqualified co-man or one without window capacity closes the path. The premium cost (premium_cost_per_unit) is an input the agent weighs against the other paths; this criterion reports availability, not whether paying the premium is the right call.
...
- call_tool(name, input): invoke a declared tool available to you (query_commitments_in_window, query_line_load, query_plants_for_sku, query_supplier_for_sku). Reader tools read world state — prefer them over inventing facts. See TOOLS AVAILABLE TO ME below.
...
- resolve_capacity_conflict
    ...
    hint: Cross-domain conflict resolution. The three context-assembly queries gather typed evidence from logistics (OTIF exposure), commercial (promo flexibility), and external co-manufacturing (availability + premium). The decision is yours — weigh the three advisory criteria against the assembled responses and pick one resolution path. The listed order of the paths is not a ranking. The orchestrator surfaces the decision to a human per its human_involvement policy on supply_planning.
...
(reader) query_line_load: Returns scheduled production load on a (plant, line) for a window, from the world-state production schedule.
    input:  LineLoadQuery (plant_code: string, line_code: string, window_start_day: integer, window_end_day: integer)
    output: LineLoad (plant_code: string, line_code: string, window_start_day: integer, window_end_day: integer, scheduled_units: decimal)
    hint: Read current scheduled load before proposing or re-proposing production on a line. The line_capacity_not_exceeded axiom evaluates the same world-state schedule deterministically; this tool lets the agent check feasibility up front so the guard passes on first try.
