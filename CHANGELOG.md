...
- **§2 framing (load-bearing).** A Playbook declares **world content** — which queries exist and are relevant (`context_assembly`), which advisory criteria apply (`decision.criteria_refs`), which resolution paths are available (`decision.selects_one_of`), which effects always fire (`always_fires`) — and **never policy** (which to prefer, in what order, what defaults). The Phase 5 stop condition stands: if Scene 5 stops producing different resolutions across LLM seeds, agency has been structured away and the playbook must be revisited against §2. Rejected field shapes at authoring time: `prefer`, `selection_order`, `fallback_if_unavailable`, `threshold`, `default_resolution`, `expected_outcome`.
- **selects_one_of neutralization.** YAML list order is authoring convenience, never priority. The renderer **alphabetizes** `context_assembly`, `criteria`, `selects_one_of`, and `always_fires`, and labels the resolution paths "(pick one; order arbitrary)". The primer states the rule explicitly. `test_resolution_paths_neutralized` is the guard.
- **Added (`scont_meta.yaml`).** Enums `PlaybookSynchronization` (`wait_all` | `wait_any`), `ToolCategory` (`reader` | `compute`). Classes `PlaybookBody` (+ `PlaybookQueryStep`, `PlaybookDecision`, `PlaybookAlwaysFires`) and `ToolBody`. `llm_prompt_hint` is a **sibling** annotation (the `FlowBody` precedent), deliberately not a body field — `extra="forbid"` rejects it in-body, and `TestPlaybookBody.test_llm_prompt_hint_in_body_rejected` pins that. Regenerated `scont_bodies.py`.
- **Added (`exploder.py`).** `scont:Playbook` / `scont:Tool` tag dispatch; `ResolvedPlaybook` / `ResolvedTool`; query API (`get_playbook`, `get_tool`, `playbooks_for_role`, `tools_for_role`); `inspect` + summary support. Cross-ref validation: playbook `role`/`triggered_by`/`input_quantum` resolve; every `context_assembly[].flow` is a **query flow** (has `returns:`); every `criteria_refs[]` resolves to an **advisory** axiom; every `selects_one_of[]` and `always_fires[]` resolves; `always_fires` entries are exactly-one-of event/flow; **single-playbook-per-(role, event)** enforced (§12.4 defaulted to single-anchor). Tool `input_class`/`output_class`/`available_to` resolve; `implementation` is a contract name that intentionally does **not** resolve in the ontology (separate from the axiom `tool_ref` registry — unifying the two is a deferred §12 question).
- **Added (`supply_chain_demo.yaml`).** Three **advisory** axioms on the Scene 5 query flows — `tolerable_otif_penalty` (on `check_otif_exposure`), `viable_promo_renegotiation` (on `check_promo_flexibility`), `viable_coman_shift` (on `check_coman_availability`) — each with an authoritative `nl:` that frames viability as a world condition and explicitly refuses to set a threshold. The `resolve_capacity_conflict` **Playbook** anchored to `(supply_planning, capacity_conflict_detected)`. Four reader **Tools** (`query_plants_for_sku`, `query_line_load`, `query_commitments_in_window`, `query_supplier_for_sku`) and their minimal input/output entity shells (`PlantQuery`, `PlantQueryResult`, `LineLoadQuery`, `LineLoad`, `CommitmentQuery`, `CommitmentQueryResult`, `SupplierQuery`).
- **Added (`ontology_service`).** `PlaybookSummary` / `ToolSummary` (+ inner views) replace the Phase-1 `Any` stubs on `RoleView`. `render_role_view` now resolves both: playbooks where `body.role == role`, tools where `role in available_to`. Two new rendered sections — **PLAYBOOKS ANCHORED TO ME** and **TOOLS AVAILABLE TO ME** — in both `as_agent_prompt()` and `as_markdown()`. The fixed `call_tool` tool-kit line now enumerates the role's available tools.
- **Changed (`ontology_primer.md`).** New "Playbook body" + "Tool body" sections; tag-dispatch table extended; navigation recipes for "what playbook fires when X at R", "which path should it pick → not in the ontology", "what tools can R call".
- **Regenerated.** All four snapshots — `supply_planning` gains the populated playbook + four-tool sections and the three advisory criteria; `production_planning`-style filtering verified (sees only `query_line_load`).
- **Tests.** 249 pass (211 + 38 new): `TestPlaybookBody` / `TestToolBody` (shapes, in-body hint rejection); `TestPlaybookCrossReferences` / `TestToolCrossReferences` (advisory-criteria requirement, query-flow requirement, single-anchor, dangling refs, `implementation` need-not-resolve); service `TestPlaybooks` / `TestTools`; render `TestPlaybookRendering` / `TestToolRendering` (incl. the §2 neutralization guard) / `TestPlaybookToolSerialization`. Integration parity counts updated (24 entities, 1 playbook, 4 tools).
- **DoD met.** `scont_meta.yaml` + `scont_bodies.py` regen clean; strict validation clean (0 warnings, 0 errors); `supply_planning.agent_prompt.txt` carries both new sections; primer updated. Explicitly **not** in scope (deferred): reader-tool implementations (orchestrator-bound at boot), `human_involvement` threshold logic (policy), `DecisionSurface` as a typed quantum (§12.3), `tool_ref`/`scont:Tool` registry unification (§12), multi-playbook composition (§12.4).
- **Pairing.** Unblocks `e2e_orchestrator` Phase 5, whose seed prompt gates on the new sections appearing in `supply_planning.agent_prompt.txt`.

### 2026-05-30 — `tool_ref` on `respect_lead_time` (orchestrator Phase 4 pairing)

- **Context.** Orchestrator Phase 4 landed the real axiom evaluator with a strict dispatch ladder: `tool_ref` → registered deterministic callable `(quantum, world_state)`; else `expr` → **slot-level subset only** (`{quantum.<slot>}` against the payload; comparisons/boolean/arithmetic; no function calls, no entity traversal); else `nl`-only advisory. `respect_lead_time`'s `expr` (`{quantum.required_by} >= today() + {quantum.supplier.lead_time_days}`) uses **both** a function call (`today()`) and entity traversal (`{quantum.supplier.lead_time_days}`) — deliberately outside the slot subset, since both are world-state access. The Phase 4 evaluator therefore reported it non-enforcing ("candidate for tool_ref") and the `RequestLifecycle` `submitted → approved` transition (guard: `respect_lead_time`) passed by default.
- **§2 framing.** `tool_ref` is world model — declares which deterministic evaluator implements the axiom, not a preference/ranking/fallback. Same shape as `lifecycle_ref`. The same reasoning that moved `line_capacity_not_exceeded` in Phase 1.7.
- **Changed.** `respect_lead_time` on `submit_procurement_request` now declares `"tool_ref": "evaluate_respect_lead_time"`. `expr` and `nl` unchanged — `expr` stays as semantic documentation, `nl` remains authoritative for reading. No schema change (`AxiomBody.tool_ref` already exists from Phase 1.7); no snapshot regen (the backbone, not the LLM, consumes `tool_ref`).
- **Orchestrator side (already done).** `evaluate_respect_lead_time` is implemented and registered in `application/axiom_tools.py::DEFAULT_AXIOM_TOOLS`; reads the injectable clock and resolves the supplier (unknown supplier → `passed=False`, `evidence="unknown_entity"`). Once the ontology declares the `tool_ref`, it routes there at runtime. After this edit, the procurement path deterministically enforces (e.g. `required_by 120 < today(100) + lead_time 28 = 128` for `SUP-MENTHOL-002` → block → `replan_on_infeasible_request`) instead of silently passing.
- **Verified.** Strict validation clean (0 warnings, 0 errors); 211 tests pass.

### 2026-05-29 — Phase 1.7: `tool_ref` on AxiomBody for world-state axioms

- **Context.** Phase 4 of `e2e_orchestrator` needs a real evaluator for `line_capacity_not_exceeded`, whose `expr` is `sum_scheduled_units(...) + volume <= rated_weekly_capacity`. `sum_scheduled_units(...)` is a function call; LinkML's `equals_expression` syntax has no arbitrary function calls. The two paths are (1) build an `expr` interpreter with `sum_scheduled_units` as a built-in — the "small language implementation" failure mode `plan_of_attack.md` Phase 4 names as a stop condition — or (2) add `tool_ref` so the orchestrator dispatches axiom evaluation to a named Python callable `(quantum_payload, world_state) → AxiomOutcome`. Path 2 is the design's intent per §12.2 and composes with the Phase 5 Tool meta-construct (both are named-callable surfaces).
- **§2 framing.** `tool_ref` is world model — it declares *which deterministic evaluator implements this axiom*, not *which axiom to prefer*. Same shape as `lifecycle_ref` (declares which FSM governs a quantum, not which transition to take). No policy.
- **Added.** Optional `tool_ref: string` on `AxiomBody` in `scont_meta.yaml`. When present, the orchestrator binds the name to a Python callable at boot and the axiom evaluator dispatches to it rather than parsing `expr`. Precedence: `tool_ref` wins over `expr` when both present; `nl` remains authoritative for human/LLM reading. Regenerated `scont_bodies.py`.
- **Changed.** `line_capacity_not_exceeded` on `request_production` now declares `"tool_ref": "evaluate_line_capacity_not_exceeded"`. `expr` stays as documentation. `respect_lead_time` is **not** retrofitted — its `expr` is slot-level and works with a simple evaluator; escalate to `tool_ref` only when needed.
- **Added.** `tests/test_bodies.py::TestAxiomBody::test_with_tool_ref` (axiom with `tool_ref` + `expr` parses; both survive) and `test_advisory_via_nl_only` (axiom with neither `tool_ref` nor `expr` still validates — advisory-via-`nl`).
- **Changed.** `ontology_primer.md` axiom-body section documents `tool_ref` and the precedence rule.
- **No snapshot changes.** `tool_ref` is for the deterministic backbone, not the LLM — the agent reads `nl` and doesn't care which evaluator runs. Role views are unchanged.
- **Non-goals (orchestrator-side, Phase 4).** The `evaluate_line_capacity_not_exceeded` callable itself is not implemented here; a hardcoded `{name: callable}` registry in the orchestrator is fine for Phase 4. Whether the name later points at a declared `scont:Tool` instance (Phase 5) is implementation detail; this PR does not couple to it.
- **Phase 1.7 DoD met.** `AxiomBody.tool_ref` is a recognized optional field; `line_capacity_not_exceeded` declares it; strict validation clean (0 warnings, 0 errors); 211 tests pass (209 + 2 new); primer updated.

### 2026-05-29 — Phase 1.6: static system-orientation preface on every role view

- **Context.** Phase 2 was verified live (e2e_orchestrator commit `ebb3302`) — the contract holds end-to-end on Gemini 2.5 Flash, the post-1.5 schema fix produces correct tool selection on the first try, and reasoning chunks are grounded in the role view ("As demand_planning, my role is to revise the forecast and then request supply…"). But the reasoning was thin. The agent knew *what its role was* from the view, but not *what kind of system it was operating in* — that routing is deterministic (so it shouldn't try to negotiate with downstream roles), that state lives elsewhere (so it shouldn't try to "remember" across invocations), that axiom rejections are the safety floor not something to argue with, that two reasoning modes exist (so when Phase 4's blocking axiom fires, it knows whether to push back). `demand_planning` has one outgoing handoff so thin reasoning sufficed; `supply_planning` in Phase 3 has multiple outgoing handoffs from one incoming flow and needs an operational stance about how to choose.
- **§2 framing.** A static orientation describing the system's operational shape — ontology = world model, agent = ephemeral, routing = deterministic, axioms = safety floor, two reasoning modes, the seven tools = action vocabulary — is **world-model commentary**, not policy. It declares what is, not what to prefer. A policy-shaped version of this ("prefer query flows before handoffs when uncertain" or "default to the first declared outgoing flow") was rejected at design time and tested for in `test_orientation_is_domain_agnostic` / `test_orientation_names_the_design_rule_and_modes`.
- **Added.** `ontology_service/orientation.py` — standalone module with the `ORIENTATION` string constant. Standalone so Phase 7 (MCP) can render the same orientation as a knowledge-worker primer without going through `as_agent_prompt`. Domain-agnostic — no mention of supply chains, OTIF, promos, etc. — so the same preface would render verbatim against a procurement, healthcare, or any other LinkML-shaped ontology following this architecture.
- **Changed.** `_render_markdown` (in `views.py`) prepends `ORIENTATION` before the `# Role:` block. `_render_agent_prompt` drops its own one-paragraph preface (`"You are an agent embedded in a supply chain coordination system…"`) — the new orientation, flowing through markdown, replaces it. Both adapters end up byte-for-byte identical in their orientation block.
- **Added.** `ORIENTATION` exported from the package (`from ontology_service import ORIENTATION`).
- **Added.** `tests/test_role_view_render.py::TestOrientation` (7 cases) — orientation appears at the top of both `as_agent_prompt` and `as_markdown` for both `demand_planning` and `supply_planning`; orientation is byte-for-byte identical across roles (the load-bearing "no per-role code" invariant); orientation contains no domain-specific terms (`supply chain`, `promo`, `otif`, `procurement`, `manufacturing`, `retailer`, `sku`, `logistics`); orientation names the world-vs-policy rule and the two reasoning modes.
- **Regenerated.** All four snapshots (`tests/snapshots/{demand_planning,supply_planning}.{agent_prompt.txt,markdown.md}`) — the orientation lands at the top of each.
- **Phase 1.6 DoD met.** Both `as_agent_prompt()` and `as_markdown()` begin with the orientation, followed by the role-specific content. Identical preface text across roles (verified). 209 tests pass (202 + 7 new); strict validation clean (0 warnings, 0 errors).
- **Pairing.** `e2e_orchestrator` requires **no changes** — the orchestrator binds whatever `as_agent_prompt()` returns. The Phase 3 session will pull the new prompt automatically when it re-imports the Ontology Service.

### 2026-05-29 — Phase 1.5: quantum + returns slot schemas in the rendered role view

- **Context.** The first live Phase 2 run (Gemini 2.5 Flash on Vertex AI) closed the demand_sensing → demand_planning → supply_planning round trip, but surfaced one renderer-side gap. The rendered system prompt told the `demand_planning` agent that its outgoing handoff `submit_supply_request` carries a `SupplyRequest`, but never declared `SupplyRequest`'s slots. The LLM guessed (`quantity` instead of `volume`, `required_by_day` instead of `required_by`, omitted `request_id`), got rejected cleanly by the deterministic validator, then retried and got lucky on attempt two. The contract held, but "the LLM guessed and got lucky" is not a verified contract — a less-guessable quantum (`OTIFExposure`, `CapacityConflict`, `ComanAvailability`) would produce retry loops or give-ups. Phase 3 adds two more LLM-driven roles emitting different quanta; without this fix every new role inherits the same dice roll.
- **§2 framing.** A class's slot structure is world model — what the action vocabulary actually carries. Excluding it from the prompt was leaving load-bearing world model out of the rendered view, exactly the failure mode the §2 rule guards against. The fix adds more of the world; no policy.
- **Added.** `QuantumSchema` and `QuantumSlotSchema` Pydantic models in `ontology_service/views.py`. `QuantumSlotSchema` carries `name`, `range`, `range_kind` ∈ {`primitive`, `class`, `enum`}, `required`, `multivalued`, `description`, and `permissible_values` (populated only for enums).
- **Added.** Optional `quantum_schema` and `returns_schema` fields on `FlowSummary`. Both reference `QuantumSchema`; `returns_schema` is non-`None` only on query flows (those with `returns:` set).
- **Added.** `_to_quantum_schema(sv, class_name)` and `_classify_range(sv, range_name)` in `ontology_service/service.py`. `_classify_range` checks enum, then class, then falls through to primitive. `_to_quantum_schema` uses `SchemaView.class_induced_slots()` to walk every slot a class induces (including inherited), defends against unknown classes by returning `None`, and emits enum permissible values inline.
- **Changed.** `_to_flow_summary(f)` → `_to_flow_summary(f, sv)`; `render_role_view` threads `self._ont.schema_view` through.
- **Changed.** `_render_section_flows` now emits a `quantum {ClassName} slots:` block under each flow, plus a `returns {ClassName} slots:` block under query flows. Class-typed slots carry an inline "Pass the entity id as a string." note; enum slots emit their permissible values inline; multivalued slots render with an `[]` array marker on the type. Optional vs. required is part of the slot line itself — the part Phase 2 showed the LLM most needs.
- **Added.** `tests/test_ontology_service.py::TestQuantumSchemas` (6 cases) — handoff carries quantum schema, class-typed slot marked `class` range_kind, query flow carries both quantum + returns schemas, enum slot carries permissible values, multivalued slot carried through (`CapacityConflict.competing_skus`), optional slot marked optional.
- **Added.** `tests/test_role_view_render.py::TestQuantumSchemaRendering` (4 cases) — handoff prompt includes the quantum slot block with required/optional and class-id nudge; query prompt includes both blocks; enum slot renders permissible values inline; multivalued slot renders the `SKU[]` marker.
- **Regenerated.** All four snapshots (`tests/snapshots/{demand_planning,supply_planning}.{agent_prompt.txt,markdown.md}`) — the drift is the point of this phase.
- **Phase 1.5 DoD met.** `render_role_view('demand_planning').as_agent_prompt()` includes slot schemas for `SupplyRequest`, `TradePromotion`, `DemandAnomaly`. `render_role_view('supply_planning').as_agent_prompt()` includes schemas for every quantum it sees as incoming or outgoing payload, plus the `returns:` classes on its three query flows (`OTIFExposure`, `PromoFlexibility`, `ComanAvailability`). 202 tests pass (192 + 10 new); strict validation clean (0 warnings, 0 errors). Phase 3's new agents now learn payload shape from the prompt instead of rolling dice.
