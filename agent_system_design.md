...
8. **The ontology exposes the handshakes (edge transport).** The world fixture and seeded boundary responders are shims for real external systems. The ontology already declares both edges — boundary roles (inbound) and `scont:Tool` (outbound) — but keeps the transport opaque (symbolic `implementation`, bound at boot; *"the agent doesn't know the difference,"* §9). Open question: should the ontology *expose the handshake* — transport (REST/MCP/A2A), endpoint, discovery — as a declared connector, or does that stay a binding/connector layer outside the ontology? Leaning: **contract in, wire out** — the typed I/O contract is world model; endpoint/auth is infrastructure, likely a thin `scont:Connector` separate from `scont:Tool`, or pure orchestrator binding. **Resolve by experiment:** Phase 7 (the MCP front door) is the first realized edge; let it inform the answer rather than deciding abstractly. A2A is the outbound/agent dual. Relates to §12.6 (boundary role implementation) and §6.2 (Tool meta-construct). **Phase 7 evidence (2026-06-01):** contract-in/wire-out *confirmed* for the inbound edge — routing + validation were already declared (`is_boundary`, quantum class + validator, router); only non-semantic wire (run id, URI scheme, retry dedup) was added. The one thing that wanted *declaring* was idempotency/session identity (it needed a home at both the wire and in-run layers) → candidate: a thin **`scont:Connector`** declaring edge-kind `{inbound|outbound}` + typed I/O + idempotency, covering both directions; endpoint/auth stayed pure config. Still open pending *outbound*-edge evidence — do not build the construct until an outbound integration adds its half. See the orchestrator repo's `briefings/phase7-live-report-mcp-front-door.md`. **Resolution (2026-06-03) — principle resolved, construct deferred.** The outbound edge is now built: Seed A's `query_baseline_demand` reader (orchestrator repo). Its `scont:Tool` declared the name + typed I/O + symbolic `implementation`; the orchestrator added only the callable + fixture read — *contract-in/wire-out confirmed a third time*. Decisively, **idempotency did not resurface**: a reader is read-only and naturally idempotent, so the one thing that wanted declaring at the inbound edge is a *command* property, not a general edge property. The 2×2 of {inbound,outbound}×{command,query} now has three cells realized — inbound-command (`ingress_quantum`), inbound-query (MCP resources), outbound-query (the reader) — all agreeing contract-in/wire-out; only **outbound-command (A2A)** is unbuilt, and it is the *sole* remaining site where idempotency could recur. **Verdict: do not build `scont:Connector`.** The principle is settled — typed I/O contract = world model (stays in the ontology); endpoint/auth/transport = wire (stays config). Idempotency/session is an inbound-command attribute: if ever declared it attaches to the boundary flow, not a new bidirectional construct. **Trigger to reopen:** the first outbound-command edge (an A2A handoff to an external agent/system) — only that can confirm or kill the bidirectional-connector idea. Building the construct before then would be the construct-first move §12.7 warns against.
